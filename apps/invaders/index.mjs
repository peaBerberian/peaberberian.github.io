...
export function create(_args, env, abortSignal) {
  ...
  const playerSprite = document.createElement("span");
  playerSprite.textContent = PLAYER_CHAR;
  let boomSvg;
  {
  ...
  applyStyle(shieldViz, {
    position: "absolute",
    borderRadius: "50%",
    border: `3px solid ${env.STYLE.windowActiveHeaderText}`,
    pointerEvents: "none",
    zIndex: "11",
    display: "none",
  ...
  applyStyle(playerSprite, {
    display: "block",
    lineHeight: "1",
    textAlign: "center",
  });
  playerElt.appendChild(playerSprite);
  ...
  lastFrameTime = now;
  tick(now);
  abortSignal?.addEventListener("abort", onDeactivate, { once: true });
  return {
    element: gameWrapper,
    onActivate,
    onDeactivate,
  ...
  function recheckGameStartupSize() {
    ...
    config = {
      ...
      enemyBulletSpeed: 0.0035 * gameState.gameHeight,
      ...
      bulletSize: Math.max(18, 0.018 * gameState.gameWidth),
  ...
  function clearBonusShip() {
    if (!gameState?.bonusShip) {
      return;
    }
    gameState.bonusShip.element.remove();
    gameState.bonusShip = null;
  }
  ...
  function setupEnemies() {
    ...
    let rows;
    let cols;
    const bossWave = gameState.level % 3 === 0;
    ...
    if (bossWave) {
      ...
      cols = Math.max(5, cols - 2);
    }
    const enemySize = config.enemySize || 25;
    const bossSize = enemySize * 2;
    const bossFormationOffset = bossWave ? bossSize * 1.65 : 0;
    ...
    if (bossWave) {
      const bossHealth = 7 + Math.floor(gameState.level / 3) * 2;
      const boss = {
        row: -1,
        col: 0,
        isUfo: true,
        ...
        diveWeight: 0,
        lastShotTs: -Infinity,
        speed: config.enemyBaseSpeed * 1.1,
        x: (gameState.gameWidth - bossSize) / 2,
        y: startY,
        width: bossSize,
        height: bossSize,
      };
      const bossElt = document.createElement("div");
      bossElt.dataset.type = "enemy";
      applyStyle(bossElt, {
        position: "absolute",
        left: boss.x + "px",
        top: boss.y + "px",
        fontSize: bossSize + "px",
        width: bossSize + "px",
        height: bossSize * 1.35 + "px",
        lineHeight: bossSize + "px",
        textAlign: "center",
        userSelect: "none",
        color: env.STYLE.windowActiveHeaderText,
      });
      const bossIcon = document.createElement("div");
      bossIcon.textContent = BOSS_CHAR;
      const bossHealthElt = document.createElement("div");
      bossHealthElt.dataset.type = "boss-health";
      applyStyle(bossHealthElt, {
        fontSize: Math.max(8, bossSize * 0.17) + "px",
        lineHeight: "1",
        whiteSpace: "nowrap",
        overflow: "hidden",
        textAlign: "center",
      });
      bossElt.appendChild(bossIcon);
      bossElt.appendChild(bossHealthElt);
      bossElt.style.animation = "circle 4s infinite linear";
      gameArea.appendChild(bossElt);
      boss.element = bossElt;
      boss.healthElement = bossHealthElt;
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        const shotWeight = row === 0 || isUfo ? 2 : 1;
        const diveWeight = row === rows - 2 ? 3 : 1;
        const enemy = {
          row,
          col,
          isUfo,
          ...
          shotWeight,
          diveWeight,
          speed: config.enemyBaseSpeed * (isUfo ? 2 : row === 0 ? 1.2 : 1),
          x: startX + col * spacing,
          y: startY + row * spacing + bossFormationOffset,
          width: enemySize,
          height: enemySize,
        };
        const enemyElt = document.createElement("div");
        enemyElt.dataset.type = "enemy";
        enemyElt.textContent = isUfo
          ? ALIEN_CHARS[1]
          : ALIEN_CHARS[row % ALIEN_CHARS.length];
        applyStyle(enemyElt, {
          position: "absolute",
          left: enemy.x + "px",
          top: enemy.y + "px",
  ...
  function updateBossHealth(boss) {
    ...
    const pips = Math.min(boss.health, 12);
    boss.healthElement.textContent = "♥".repeat(pips);
  }
  ...
  function trySpawnBonusShip() {
    ...
    const size = config.enemySize * 1.35;
    ...
    bonusShip.element.dataset.type = "bonus-ship";
    bonusShip.element.textContent = BONUS_SHIP_CHAR;
    applyStyle(bonusShip.element, {
      position: "absolute",
      left: bonusShip.x + "px",
      top: bonusShip.y + "px",
      width: size + "px",
    ...
    gameArea.appendChild(bonusShip.element);
    gameState.bonusShip = bonusShip;
  }

  function updateBonusShip() {
    trySpawnBonusShip();
    const ship = gameState.bonusShip;
    if (!ship) {
      return;
    }
    ship.x += ship.vx;
  ...
  function trySpawnDiver() {
    ...
    const source = pickWeightedFrom(candidates, "diveWeight");
    const idx = gameState.enemies.indexOf(source);
  ...
  function pickWeightedFrom(enemies, weightProp) {
    const total = enemies.reduce(
      (sum, enemy) => sum + (enemy[weightProp] ?? 1),
      0,
    );
    let cursor = Math.random() * total;
    for (const enemy of enemies) {
      cursor -= enemy[weightProp] ?? 1;
      if (cursor <= 0) {
        return enemy;
      }
    }
      ...
      if (
        ...
        if (now - gameState.player.lastDamageTs > 600) {
          if (gameState.activePowerups.SHIELD > now) {
            ...
            showPowerupLabel("SHIELD BROKEN!");
          } else {
            gameState.player.lives--;
            gameState.player.lastDamageTs = now;
            triggerScreenShake();
            audio.playSound("damage");
            if (gameState.player.lives <= 0) endGame();
          }
        }
        spawnParticles(
  ...
  function updatePlayer() {
    ...
    if (mousePressed || FIRE_KEYS.some((k) => currentlyPressedKeys[k])) {
      shoot();
    }
  }
  ...
  function updateBullets() {
    ...
    gameState.enemyBullets = gameState.enemyBullets.filter((bullet) => {
      bullet.x += bullet.vx || 0;
      bullet.y += bullet.vy || config.enemyBulletSpeed;
      if (
        bullet.y >= gameState.gameHeight + bullet.height ||
        bullet.x < -bullet.width ||
        bullet.x > gameState.gameWidth + bullet.width
      ) {
        bullet.element.remove();
  ...
  function updateEnemies() {
    if (gameState.enemies.length === 0 && gameState.divers.length === 0) {
      ...
      clearBullets();
      setupEnemies();
      gameState.waveReadyAt = performance.now() + 900;
      const isBossWave = gameState.level % 3 === 0;
      showPowerupLabel(
        isBossWave ? `BOSS WAVE ${gameState.level}` : `WAVE ${gameState.level}`,
      );
      audio.playSound(isBossWave ? "boss" : "wave");
    ...
    if (canShoot && gameState.enemies.length > 0 && Math.random() < shootChance) {
      ...
      if (shooter.isBoss) {
        ...
        const bossCooldown = Math.max(760, 1300 - bossNumber * 160);
        if (now - shooter.lastShotTs > bossCooldown) {
          spawnBossBullets(shooter);
  ...
  function spawnEnemyBullet(shooter, vx = 0, vy = config.enemyBulletSpeed) {
  ...
  function spawnBossBullets(boss) {
    const speed = config.enemyBulletSpeed * (0.95 + gameState.level * 0.02);
    const pattern = [-0.58, -0.2, 0.2, 0.58];
    for (const angle of pattern) {
      spawnEnemyBullet(boss, Math.sin(angle) * speed, Math.cos(angle) * speed);
    }
  }

  function pickWeightedEnemy(weightProp) {
    const total = gameState.enemies.reduce(
      (sum, enemy) => sum + (enemy[weightProp] ?? 1),
      0,
    );
    let cursor = Math.random() * total;
    ...
    return gameState.enemies[gameState.enemies.length - 1];
  }
  ...
  function checkCollisions() {
    ...
    for (let i = gameState.bullets.length - 1; i >= 0; i--) {
      ...
      if (gameState.bonusShip && rectsOverlap(bullet, gameState.bonusShip)) {
        ...
        spawnParticles(
          gameState.bonusShip.x + gameState.bonusShip.width / 2,
          gameState.bonusShip.y + gameState.bonusShip.height / 2,
          env.STYLE.windowActiveHeaderText,
        );
        clearBonusShip();
        scheduleNextBonusShip();
  ...
  function render() {
    ...
    if (gameState.gameWidth === 0 || gameState.gameHeight === 0) {
      ...
      hud.textContent = `SCORE: ${gameState.score} | BEST: ${highScore} | ${hearts} | WAVE: ${gameState.level}`;
    ...
    if (gameState.gameOver) {
      ...
      gameOverScreen.innerHTML = `GAME OVER<br>FINAL SCORE: ${gameState.score}<br>BEST: ${highScore}<br><span style="font-size: ${config.hudSize}px;">Press R or Click to restart</span>`;
    } else {
      if (!gameState.started) {
        if (gameState.gameWidth === 0 || gameState.gameHeight === 0) {
          ...
          startScreen.innerHTML = `Invaders!<br><span style="font-size: ${config.hudSize}px;">Move: A/D or arrows<br>Fire: Space, Up, or hold click</span>`;
        }
      } else {
        startScreen.style.display = "none";
      }
  ...
  function onKeyDown(e) {
    ...
    if (FIRE_KEYS.includes(keyPressed)) {
      e.preventDefault();
      if (!gameState.started) {
  ...
  function onClick() {
    audio.ensureAudio();
    if (gameState.gameOver) {
      if (performance.now() - gameState.gameOverAt < 300) {
        return;
      }
      restartGame();
      gameState.started = true;
