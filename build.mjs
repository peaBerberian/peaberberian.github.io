...
/** Base source directory for the desktop code. */
const DESKTOP_SRC_DIR = path.join(PROJECT_ROOT_DIRECTORY, "src");

/**
 * Base source directory for default apps that should be built.
 *
 * An `AppInfo.json` config file should be present at the root of that directory
 * to know which applications should be builts and what their requirements are.
 */
const APPS_SRC_DIR = path.join(PROJECT_ROOT_DIRECTORY, "apps");

/** Base entry point for the desktop code source. */
const DESKTOP_MAIN = path.join(DESKTOP_SRC_DIR, "desktop.mjs");

/** Base entry point for the "sandboxed" apps sandbox code's source. */
const SANDBOX_MAIN = path.join(DESKTOP_SRC_DIR, "app-lib/app-sandbox.mjs");

/**
 * Outputed `__generated_apps.mjs` file which the desktop will use to know about
 * "pre-installed" apps.
 *
 * This file will be created by this build script from the `AppInfo.json` config
 * file.
 */
const GENERATED_APP_PATH = path.join(DESKTOP_SRC_DIR, "__generated_apps.mjs");

/** The base directory where all bundled code will be outputed. */
const OUTPUT_DIR = path.join(PROJECT_ROOT_DIRECTORY, "dist");

/** Stylesheet inserted for sandboxed apps. Should already be generated. */
const APP_STYLE = path.join(OUTPUT_DIR, "app-style.css");

/**
 * Directory where bundled apps will be put.
 *
 * They are completely generated by this script.
 */
const OUTPUT_LAZY_LOADED_APPS = path.join(OUTPUT_DIR, "lazy");

/**
 * Output file where the desktop bundle will be set.
 *
 * Is completely generated by this script.
 */
const OUTPUT_DESKTOP = path.join(OUTPUT_DIR, "desktop.js");

/**
 * Special output script relied on by "sandboxed" app.
 *
 * Is completely generated by this script.
 */
...
/**
 * Output HTLM app for the fully bundled "sandbox", which is the page
 * where "sandboxed" apps will be loaded.
 *
 * Is completely generated by this script.
 */
const SANDBOX_HTML = path.join(OUTPUT_DIR, "sandbox.html");
 ...
 * @param {Object} [options.appBaseUrl] - If set, the "sandboxed" applications
 * are stored in a completely different directory  from the desktop and this is
 * the base url (should **NOT** end with a `/`) where apps will be loaded.
 ...
 * a key-value objects, where the object is a string (trick: if you want to
 * replace an identifier with a string, call `JSON.stringify` on it).
 * @returns {Promise} - Promise resolving when a bundle has been generated (the
 * first one if watching). Reject if it could not generate a build and will not
 * be able to do so even if watching and the issue is fixed.
 */
...
/**
 * Write "sandbox" HTML file dynamically and, if `watch` is set, regenerate it
 * every time one of its source file is updated.
...
async function writeAppSandboxHtml(watch, isSilent) {
  ...
  if (watch) {
    [APP_STYLE, OUTPUT_APP_LIB_SCRIPT].forEach((filePath) => {
      ...
      async function onFileChange(eventName) {
        if (eventName !== "change" || alreadyWaitingToBuild) {
          return;
        }
  ...
  async function _writeSandbox() {
    const readSndbxScript = new Promise((resolve, reject) =>
      fs.readFile(OUTPUT_APP_LIB_SCRIPT, function (err, val) {
        if (err) {
          reject(err);
          return;
        }
    ...
    const readCss = new Promise((resolve, reject) =>
      fs.readFile(APP_STYLE, function (err, val) {
        if (err) {
          reject(err);
          return;
        }
