/** This is the code for the app identified as "this-website". */
var p="https://github.com/peaberberian/peaberberian.github.io";function m(c,o){let{createAppTitle:i}=o.appUtils;return{sidebar:[{text:"Overview",centered:!0,icon:"\u{1F9D1}\u200D\u{1F3EB}",render:a},{text:"Making it efficient",centered:!0,icon:"\u{1F3CE}\uFE0F",render:n},{text:"Making it configurable",centered:!0,icon:"\u{1F9F0}",render:s},{text:"Making it secure",centered:!0,icon:"\u{1F510}",render:r},{text:"Why so many emojis?",centered:!0,icon:"\u{1F635}\u200D\u{1F4AB}",render:l},{text:"Some Anecdotes",centered:!0,icon:"\u{1F388}",render:h},{text:"External Resources",centered:!0,icon:"\u{1FAF6}",render:d}]};function a(){let e=document.createElement("div");e.appendChild(i("About This Website",{github:p}));let t=document.createElement("div");return e.appendChild(t),t.innerHTML=`<p>I initially envisionned a simple desktop environment where I could list my open-source projects to replace my previously minimal (some said bland) personal website.</p>
...
<p>So there has been a change of plan: it's not a listing of other projects anymore, now it's its own thing as a JS desktop environment!</p>`,e}function n(){let e=document.createElement("div");e.appendChild(i("Making it efficient",{}));let t=document.createElement("div");return e.appendChild(t),t.innerHTML=`<p>I tried to make that website as efficient as possible:</p>
<ul>
...
</ul>`,e}function s(){let e=document.createElement("div");e.appendChild(i("Making it configurable",{}));let t=document.createElement("div");return e.appendChild(t),t.innerHTML=`<p>I spent some efforts trying to make the behavior of this desktop configurable (through the settings app).</p>
...
</div>`,e}function r(){let e=document.createElement("div");e.appendChild(i("Making it secure",{}));let t=document.createElement("div");return e.appendChild(t),t.innerHTML=`<p>Even though this is a simple personal project, this desktop is actually developed with some high security constraints.</p>
...
</div>`,e}function l(){let e=document.createElement("div");e.appendChild(i("So many emojis \u{1F635}\u200D\u{1F4AB}",{}));let t=document.createElement("div");return e.appendChild(t),t.innerHTML=`<p>As you can see, I rely heavily on emojis for many picture-like aspects of this website (icons, sidebar categories, start button, theming).</p>
...
<p>It could be argued that some devices don't have any font with emojis, though those should generally not be devices from the target audience of that website. Yet if it turns out that a visitor still don't have them, the fallback font or replacement characters will make it clear that the current device is missing them.</p>`,e}function h(){let e=document.createElement("div");e.appendChild(i("Some anecdotes",{}));let t=document.createElement("div");return e.appendChild(t),t.innerHTML=`<p>While writing this, I encountered several difficulties that I found interesting.</p>
...
<p>But doing that weirdly led to what appeared to be some remnants of the old color at the edges.<br>After thinking my code was wrong and trying to fix some stuff, I finally looked at what exact pixels where not filled: turned out that they had a weird "color pixel" that I had never drawn, WTF \u{1F62E}...</p>
...
</ol>`,e}function d(){let e=document.createElement("div");e.appendChild(i("External Resources",{}));let t=document.createElement("div");return e.appendChild(t),t.innerHTML=`<p>I had to find many images to write this project.<br>I chose to only include resources with a very permissive license: CC0 and public domain only.</p>
...
<p>For icons, I mainly sourced it from  <a href="https://www.svgrepo.com/" target="_blank">svgrepo</a>, in particular its  <a href="https://www.svgrepo.com/collection/minimal-ui-icons/" target="_blank">Minimal UI Icon</a> collection. Thanks to the people involved in that set!</p>`,e}}export{m as create};
