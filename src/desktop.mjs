...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const openAppFromIconOrStartMenu = (appObj, appArgs) => {
    createApp(appObj, appArgs ?? [], {
      activate: true,
      fullscreen: false,
      skipAnim: false,
      centered: false,
   ...
   */
  function createApp(app, appArgs, options) {
    const windowElt = windowsManager.openApp(app, appArgs, options);
    if (windowElt !== null) {
  ...
  for (const app of apps) {
    if (app.desktopDir !== undefined) {
      ...
      if (list === undefined) {
        ...
        appIcons.push(generateAppGroupApp(app.desktopDir, list, onOpen));
      }
      list.push(app);
    } else {
      appIcons.push(app);
